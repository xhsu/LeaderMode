...
void mpcontrol_t::OnSizeChanged(int wide, int tall)
{
	...
	if (pPrompt)
	{
		...
		pPrompt->SetBounds(0, inset, w + 14, tall - 2 * inset);
	}
	else
	{
		if (pControl)
...
objtype_t CScriptObject::GetType(char *pszType)
{
	...
	{
		if (!Q_stricmp(objtypes[i].szDescription, pszType))
...
CScriptObject *CDescription::FindObject(const char *pszObjectName)
{
	...
	while (p)
	{
		if (!Q_stricmp(pszObjectName, p->cvarname))
			return p;
...
bool CDescription::ReadFromBuffer(char **pBuffer)
{
	...
	*pBuffer = gEngfuncs.COM_ParseFile(*pBuffer, token);
	...
	if (Q_stricmp(token, m_pszDescriptionType))
	{
...
void CDescription::setDescription(const char *pszDesc)
{
	...
	m_pszDescriptionType = Q_strdup(pszDesc);
}
...
void CDescription::setHint(const char *pszHint)
{
	...
	m_pszHintText = Q_strdup(pszHint);
}
