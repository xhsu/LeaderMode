...
{
	if ( m_bIsCommand )
		return gEngfuncs.GetCmdFunctionName(m_pCommand);
...
static unsigned int FindNamedCommand( const char *name )
{
	for ( unsigned int cmd = gEngfuncs.GetFirstCmdFunctionHandle(); cmd; cmd = gEngfuncs.GetNextCmdFunctionHandle( cmd ) )
	{
		if ( !Q_strcmp( gEngfuncs.GetCmdFunctionName( cmd ), name ) )
			return cmd;
	}
...
void CConsolePanel::RebuildCompletionList(const char *text)
{
	...
	if ( space )
	{
		unsigned int pCommand = FindAutoCompleteCommmandFromPartial( text );
		if ( !pCommand )
			return;
	...
	if ( bNormalBuild )
	{
		...
		while (cmd)
		{
			if ( !Q_strnicmp( text, gEngfuncs.GetCmdFunctionName( cmd ), len ) )
			{
				...
				item->m_pCommand = cmd;
				item->m_pCvars = NULL;
				const char *tst = gEngfuncs.GetCmdFunctionName( cmd );
		...
		cvar_t *var = gEngfuncs.GetFirstCvarPtr();

		while ( var )
		{
			if ( !Q_strnicmp( text, var->name, len ) )
			{
				CompletionItem *item = new CompletionItem();
				m_CompletionList.AddToTail( item );
...
void CConsolePanel::OnAutoComplete(bool reverse)
{
	...
	{
		Q_strncat(completedText, " ", sizeof(completedText));
	}
