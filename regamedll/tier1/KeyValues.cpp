	...
	void ReportError( const char *pError )
	{
		...
		for ( int i = 0; i < m_maxErrorIndex; i++ )
		{
			if ( m_errorStack[i] != INVALID_KEY_SYMBOL )
			{
				if ( i < m_errorIndex )
				{
					Warning( "%s, ", g_pKeyValuesSystem->GetStringForSymbol(m_errorStack[i]) );
				}
				else
				{
					Warning( "(*%s*), ", g_pKeyValuesSystem->GetStringForSymbol(m_errorStack[i]) );
...
{
	return g_pKeyValuesSystem->GetStringForSymbol(m_iKeyName);
}
...
bool KeyValues::LoadFromFile( IFileSystem *filesystem, const char *resourceName, const char *pathID )
{
	...
	bool bRetOK = ( ((IFileSystem *)filesystem)->Read( buffer, fileSize, f ) != 0 );
...
KeyValues *KeyValues::FindKey(const char *keyName, bool bCreate)
{
	...
	HKeySymbol iSearchStr = g_pKeyValuesSystem->GetSymbolForString( searchStr );
...
{
	m_iKeyName = g_pKeyValuesSystem->GetSymbolForString( setName );
}
...
{
	MEM_ALLOC_CREDIT();
	return g_pKeyValuesSystem->AllocKeyValuesMemory(iAllocSize);
}

void *KeyValues::operator new( unsigned int iAllocSize, int nBlockUse, const char *pFileName, int nLine )
{
	void *p = g_pKeyValuesSystem->AllocKeyValuesMemory(iAllocSize);
	return p;
}
...
{
	g_pKeyValuesSystem->FreeKeyValuesMemory(pMem);
}
