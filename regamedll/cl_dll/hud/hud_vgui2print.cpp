...
int CHudVGUI2Print::VidInit(void)
{
	...
	m_hudfont = VGUI_SCHEME->GetIScheme(VGUI_SCHEME->GetDefaultScheme())->GetFont("EngineFont", true);
	return 1;
}
...
int CHudVGUI2Print::DrawVGUI2StringReverse(wchar_t* msg, int x, int y, float r, float g, float b)
{
	...
	if (y == -1)
	{
		fontheight = VGUI_SURFACE->GetFontTall(m_hudfont);
	...
	for (i = len - 1; i >= 0; i--)
	{
		...
		VGUI_SURFACE->GetCharABCwide(m_hudfont, msg[i], w1, w2, w3);
...
int CHudVGUI2Print::DrawVGUI2StringReverse(char* charMsg, int x, int y, float r, float g, float b)
{
	...
	{
		wchar_t buf[512];
		VGUI_LOCALISE->ConvertANSIToUnicode(charMsg, buf, sizeof(buf));
		msg = buf;
	}
...
void CHudVGUI2Print::VGUI2LocalizeArg(char* pSrc, wchar_t* pDest, int unicodeBufferSizeInBytes)
{
	...
	if (pSrc[0] == '#')
	{
		wchar_t* temp = VGUI_LOCALISE->Find(pSrc);
...
void CHudVGUI2Print::VGUI2HudPrintArgs(char* charMsg, char* sstr1, char* sstr2, char* sstr3, char* sstr4, int x, int y, float r, float g, float b)
{
	...
	msg = VGUI_LOCALISE->Find(charMsg);

	if (msg)
	{
		VGUI_LOCALISE->ConstructString(m_wCharBuf, sizeof(m_wCharBuf), msg, 4, wstr1, wstr2, wstr3, wstr4);
	}
	else
	{
		VGUI_LOCALISE->ConvertANSIToUnicode(charMsg, m_wCharBuf, sizeof(m_wCharBuf));
	}
}
...
int CHudVGUI2Print::GetHudFontHeight(void)
{
	...
	return VGUI_SURFACE->GetFontTall(m_hudfont);
}
...
void CHudVGUI2Print::GetStringSize(const wchar_t* string, int* width, int* height)
{
	...
	if (width)
	{
		for (i = 0; i < len; i++)
		{
			VGUI_SURFACE->GetCharABCwide(m_hudfont, string[i], a, b, c);
			*width += a + b + c;
		}
	}
...
int CHudVGUI2Print::DrawVGUI2String(wchar_t* msg, int x, int y, float r, float g, float b)
{
	...
	for (i = 0; i < iTotalLines; i++)
	{
		...
		for (j = 0; j < iTempCount; j++)
		{
			...
			VGUI_SURFACE->GetCharABCwide(m_hudfont, line[j], w1, w2, w3);
