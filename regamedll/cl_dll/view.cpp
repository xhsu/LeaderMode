...
==============
*/
void V_AddIdle(ref_params_s* pparams)
{
...
You would seeing this after the game match.
The sight with a scoreboard floating on the floor is this to blame.
==============
*/
void V_CalcIntermissionRefdef(ref_params_s* pparams)
{
	cl_entity_t* pViewEntity;
	float		old;

	// pViewEntity is the weapon model (only visible from inside body)
	pViewEntity = gEngfuncs.GetViewModel();
...
Get into an entity's eye. Normally used on player.
==================
*/
...
==================
*/
int V_FindViewModelByWeaponModel(int iModelIndex)
{
	model_t* pWeaponModel = IEngineStudio.GetModelByIndex(iModelIndex);
...
void V_GetChaseOrigin(float* angles, float* origin, float distance, float* returnvec)
{
	...
	v_lastDistance = (Vector(trace->endpos) - Vector(origin)).Length();	// real distance without offset
}
...
void V_GetDeathCam(cl_entity_t* ent1, cl_entity_t* ent2, float* angle, float* origin)
{
	float newAngle[3];
	float newOrigin[3];
	static Vector nonDestructedOrigin;
...
void V_GetMapFreePosition(float* cl_angles, float* origin, float* angles)
{
	...
	*/
	...
	//VectorMA(zScaledTarget, -(4096.0f / gHUD.m_Spectator.m_mapZoom), forward, origin); UNDONE
}
...
void V_CalcSpectatorRefdef(ref_params_s* pparams)
{
	...
	if ((g_iUser1 == OBS_IN_EYE/* || gHUD.m_Spectator.m_pip->value == INSET_IN_EYE*/) && ent)	// UNDONE
	{
...
==================
*/
float V_CalcBob(ref_params_s* pparams)
{
...
==============
*/
void V_CalcViewRoll (ref_params_s* pparams)
{
	float		side;
